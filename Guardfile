...
#  .select{|d| Dir.exist?(d) ? d : UI.warning("Directory #{d} does not 
exist")}
...
# force_run: false                     # kills any process that's holding 
the listen port before attempting to (re)start Rails.
# pid_file: 'tmp/pids/[RAILS_ENV].pid' # specify your pid_file.
# host: 'localhost'                    # server hostname.
# port: 3000                           # server port number.
# root: '/spec/dummy'                  # Rails' root path.
# server: thin                         # webserver engine.
# start_on_start: true                 # will start the server when 
starting Guard.
...
# Note: The cmd option is now required due to the increasing number of 
ways
