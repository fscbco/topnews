...
describe FetchTopStories do
  describe ".call" do
    it "gets and creates stories from the hacker news api" do
      ...
      allow(hacker_news_api).to receive(:get_story).with(5).and_return(
        {"by" => "author5", "title" => "title5", "url" => "url5"}
      )
      allow(hacker_news_api).to receive(:get_story).with(6).and_return(
        {"by" => "author6", "title" => "title6", "url" => "url6"}
      )
